...
```txt
sudo sed -i 's/bfd/lld/g' $(ls /usr/src/nvidia-*/dkms.conf)
```
也可以使用vscode打开文件，Ctrl+F搜索bfd 改成lld即可，然后安装三个deb包。注意，不要安装带有dbg字符串的包，除非你知道自己在干什么。先安装linux-headers-开头的deb，其他的两个linux-image-开头的和linux-libc开头的deb包安装顺序无所谓。
